...
async function getLivePricePerHour(
  serviceType: string,
): Promise<number | null> {
  try {
    ...
    const services: Array<{ type: string; pricePerHour: number }> =
      await res.json();
...
export default async function ServicePage({
  ...
  const rate =
    livePrice != null
      ? `₹${livePrice.toLocaleString("en-IN")} / hour`
      : service.rate;
