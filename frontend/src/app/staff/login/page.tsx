  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] dark:bg-[#111111] flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-xs font-bold tracking-widest uppercase text-gray-500 dark:text-white mb-3">{portalLabel}</p>
        ...
        <div className="card">
          ...
          <p className="text-center text-xs text-[#888] dark:text-[#999] mt-6">
            {staffRole === 'STUDIO_MANAGER' ? (
              <>Not a manager? <button type="button" onClick={() => { setStaffRole('EMPLOYEE'); loginForm.reset(); }} className="text-[#E5312A] hover:underline">Employee login</button></>
            ) : (
              <>Not an employee? <button type="button" onClick={() => { setStaffRole('STUDIO_MANAGER'); loginForm.reset(); }} className="text-[#E5312A] hover:underline">Studio Manager login</button></>
            )}
          </p>
        </div>
      </div>
