...
interface Payment {
  id: string; type: string; mode: string;
  amount: number; status: string; paidAt: string | null;
}

interface Invoice {
  id: string; invoiceNumber: string; totalAmount: number; cloudinaryUrl: string | null;
}

interface Employee {
  id: string; user?: { name: string; email: string };
}
...
export default function BookingDetailPage() {
  ...
  const handlePayOnline = async () => {
    ...
    try {
      ...
      if (!ok) { toast.error('Payment gateway unavailable. Try again later.'); return; }
      ...
      const rzp = new (window as any).Razorpay({
        ...
        handler: async (resp: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          try {
            ...
            toast.error(`Payment received but verification failed. Share Payment ID ${resp.razorpay_payment_id} with the studio team.`);
          }
        },
      });
      rzp.on('payment.failed', () => toast.error('Payment failed. Please try a different payment method.'));
      rzp.open();
    } catch (err: unknown) {
      const msg = err && typeof err === 'object' && 'response' in err
        ? (err as any).response?.data?.message
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-sm text-[#6b6b6b] dark:text-[#8a8a8a]">Loading booking…</span>
      </div>
    );
  }
  ...
  if (!booking) {
    return (
      <div className="text-center py-20">
        <p className="font-bold text-gray-900 dark:text-white mb-2">Booking not found</p>
        <button onClick={() => router.back()} className="text-[#E5312A] text-sm hover:underline">Go back</button>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6b6b6b] dark:text-[#8a8a8a] mt-1">{booking.service?.name}</p>
        </div>
        <span className={`inline-block px-3 py-1.5 text-[10px] font-black tracking-[0.15em] uppercase self-start ${meta.cls}`}>
          {meta.label}
        </span>
      </div>
      ...
      {user?.role === 'CUSTOMER' && status === 'ADVANCE_PAID' && (
        <div className="border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/10 p-4 flex items-center gap-3">
          ...
          <p className="text-sm font-medium text-green-800 dark:text-green-400">
            Advance payment received — your slot is confirmed. We'll be in touch before the shoot day.
          </p>
        </div>
      )}
      ...
      {user?.role === 'CUSTOMER' && confirmCancel && (
        <div className="border border-[#E5312A] bg-[#E5312A]/5 p-4 flex items-start gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-gray-900 dark:text-white mb-1">Cancel this booking?</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <SectionCard title="Shoot Details">
            <dl>
              <DetailRow label="Date" value={
                new Date(booking.shootDate).toLocaleDateString('en-IN', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })
              } />
              <DetailRow label="Time Slot" value={`${booking.startTime} – ${booking.endTime}`} />
              <DetailRow label="Duration" value={`${booking.durationHours} hours`} />
              <DetailRow label="Service" value={booking.service?.name} />
              {booking.studioRequirements && <DetailRow label="Studio Setup" value={booking.studioRequirements} />}
              {booking.equipmentRequired  && <DetailRow label="Equipment" value={booking.equipmentRequired} />}
              {booking.additionalNotes    && <DetailRow label="Notes" value={booking.additionalNotes} />}
            </dl>
          </SectionCard>
          ...
          <SectionCard title="Customer">
            <dl>
              <DetailRow label="Name"    value={booking.customerName || booking.customer?.user?.name || booking.createdBy?.name} />
              <DetailRow label="Email"   value={booking.customerEmail || booking.customer?.user?.email || booking.createdBy?.email} />
              <DetailRow label="Phone"   value={booking.customerPhone || booking.customer?.user?.phone} />
              {booking.companyName && <DetailRow label="Company" value={booking.companyName} />}
              {booking.agent && <DetailRow label="Agent" value={booking.agent.user?.name} />}
            </dl>
          </SectionCard>
                ...
                <table className="w-full text-sm">
                  <thead className="border-b border-[#e5e5e5] dark:border-[#2a2a2a] bg-[#f5f5f5] dark:bg-[#181818]">
                    <tr>
                      {['Type', 'Mode', 'Amount', 'Status', 'Date'].map(h => (
                        <th key={h} className="text-left px-4 py-2.5 text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#f5f5f5] dark:divide-[#1a1a1a]">
                    {booking.payments.map(p => (
                      <tr key={p.id}>
                        <td className="px-4 py-3 text-gray-900 dark:text-white text-sm">{p.type}</td>
                        <td className="px-4 py-3 text-[#6b6b6b] dark:text-[#8a8a8a] text-sm">{p.mode}</td>
                        <td className="px-4 py-3 font-bold text-gray-900 dark:text-white text-sm">₹{Number(p.amount).toLocaleString('en-IN')}</td>
                        <td className="px-4 py-3">
                          <span className={`text-[10px] font-black tracking-[0.1em] uppercase px-2 py-1 ${p.status === 'PAID' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400'}`}>
                            {p.status}
                          </span>
                        </td>
            ...
            <SectionCard title="Invoices">
              <div className="space-y-0 border border-[#e5e5e5] dark:border-[#2a2a2a]">
                {booking.invoices.map(inv => (
                  <div key={inv.id} className="flex items-center justify-between px-4 py-3 border-b border-[#f5f5f5] dark:border-[#1a1a1a] last:border-0">
                    <div>
          ...
          <div className="space-y-3">
                ...
                {showQuote ? (
                  <form onSubmit={handleSubmit(d => action(() => api.patch(`/bookings/${id}/quote`, d), 'Quote sent to customer').then(() => setShowQuote(false)))} className="space-y-3">
                    ...
                    <div>
                      ...
                      <input {...register('discountAmount')} type="number" className="input-field" placeholder="0" />
                    </div>
                    <div className="flex gap-2 pt-1">
                      <button type="submit" disabled={actionLoading} className="flex-1 bg-[#E5312A] hover:bg-[#CC2A24] text-white text-xs font-bold py-2.5 transition-colors disabled:opacity-50">
                        {actionLoading ? 'Sending…' : 'Send Quote'}
                      </button>
                      <button type="button" onClick={() => setShowQuote(false)} className="flex-1 border border-[#e5e5e5] dark:border-[#2a2a2a] text-xs font-bold text-[#6b6b6b] dark:text-[#8a8a8a] py-2.5 hover:border-[#aaa] transition-colors">
                        Cancel
                      </button>
                    </div>
                  </form>
                ) : (
                  <button onClick={() => setShowQuote(true)} className="w-full bg-[#E5312A] hover:bg-[#CC2A24] text-white text-sm font-bold py-2.5 transition-colors">
                    Create Quote
                  </button>
                )}
              </SectionCard>
            ...
            {status === 'QUOTED' && (
              <SectionCard title="Approve Booking">
                ...
                <button onClick={() => action(() => api.patch(`/bookings/${id}/approve`), 'Booking approved')} disabled={actionLoading} className="w-full bg-[#E5312A] hover:bg-[#CC2A24] text-white text-sm font-bold py-2.5 transition-colors disabled:opacity-50">
            ...
            {['APPROVED', 'ADVANCE_PAID', 'IN_PROGRESS'].includes(status) && (
              <SectionCard title="Assign Crew">
                <select value={assignEmp} onChange={e => setAssignEmp(e.target.value)} className="input-field mb-3 text-sm">
                  <option value="">Select employee</option>
                  {employees.map(emp => (
                    <option key={emp.id} value={emp.id}>{emp.user?.name ?? emp.id}</option>
                  ))}
                </select>
                <button onClick={() => { if (!assignEmp) return toast.error('Select an employee first'); action(() => api.patch(`/bookings/${id}/assign-employee`, { employeeId: assignEmp }), 'Employee assigned'); }} disabled={actionLoading} className="w-full bg-[#E5312A] hover:bg-[#CC2A24] text-white text-sm font-bold py-2.5 transition-colors disabled:opacity-50">
                  {booking.employee ? 'Reassign' : 'Assign'}
                </button>
                {booking.employee && (
                  <p className="text-xs text-[#6b6b6b] dark:text-[#8a8a8a] mt-2">
                    Currently assigned: <strong className="text-gray-900 dark:text-white">{booking.employee.user?.name}</strong>
                  </p>
                )}
              </SectionCard>
            )}
            ...
            {status === 'ADVANCE_PAID' && (
              <SectionCard title="Start Shoot">
                <button onClick={() => action(() => api.patch(`/bookings/${id}/start`), 'Shoot started')} disabled={actionLoading} className="w-full bg-[#E5312A] hover:bg-[#CC2A24] text-white text-sm font-bold py-2.5 transition-colors disabled:opacity-50">
            ...
            {status === 'IN_PROGRESS' && (
              <SectionCard title="Complete Booking">
                <button onClick={() => action(() => api.patch(`/bookings/${id}/complete`), 'Booking marked as completed')} disabled={actionLoading} className="w-full bg-[#E5312A] hover:bg-[#CC2A24] text-white text-sm font-bold py-2.5 transition-colors disabled:opacity-50">
            ...
            {(status === 'ADVANCE_PAID' || status === 'COMPLETED') && (
              <SectionCard title="Invoice">
                <button onClick={() => action(() => api.post('/invoices/generate', { bookingId: id, type: 'GST_INVOICE' }), 'GST invoice generated')} disabled={actionLoading} className="w-full border border-[#e5e5e5] dark:border-[#2a2a2a] text-sm font-bold text-gray-900 dark:text-white py-2.5 hover:bg-[#f5f5f5] dark:hover:bg-[#181818] transition-colors flex items-center justify-center gap-2">
            ...
            {!['COMPLETED', 'CANCELLED'].includes(status) && (
              <div className="border border-[#E5312A]/30 bg-white dark:bg-[#111111] p-4">
                <p className="text-[10px] font-black tracking-[0.15em] uppercase text-[#E5312A] mb-3">Danger Zone</p>
                {confirmCancel ? (
                  ...
                  <button onClick={() => setConfirmCancel(true)} className="w-full border border-[#E5312A]/30 text-[#E5312A] text-sm font-bold py-2.5 hover:bg-[#E5312A]/5 transition-colors">
                    Cancel Booking
                  </button>
                )}
              </div>
