...
export default function NewBookingPage() {
  ...
  const totalAmount =
    selectedService && duration
      ? Math.round(duration * selectedService.pricePerHour)
      : null;
  ...
  const checkAvailability = (st: string, et: string, sd: string) => {
    ...
    setChecking(true);
    api
      .get<AvailabilityResult>("/bookings/availability", {
        params: { date: sd, startTime: st, endTime: et },
      })
      .then((r) => {
        if (reqId === availabilityReqId.current) setAvailability(r.data);
      })
      .catch(() => {
        if (reqId === availabilityReqId.current) setAvailability(null);
      })
      .finally(() => {
        if (reqId === availabilityReqId.current) setChecking(false);
      });
  };
  ...
  const onSubmit = async (data: FormData) => {
    if (availability && !availability.available) {
      toast.error(
        "This slot is already taken. Please choose a different time.",
      );
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.success(
        "Booking confirmed! You can pay online or offline from the booking page.",
      );
            ...
            <select {...register("serviceId")} className="input-field">
              ...
              {services.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.name}: ₹{Number(s.pricePerHour).toLocaleString("en-IN")}/hr
                  · min {s.minDuration}h
                </option>
              ))}
            </select>
          ...
          {startTime && endTime && (
            <div className="flex flex-wrap items-center gap-5 border border-[#e5e5e5] dark:border-[#2a2a2a] px-4 py-3 bg-[#f5f5f5] dark:bg-[#181818]">
              ...
              {checking ? (
                ...
                <span
                  className={`flex items-center gap-1.5 text-sm font-bold ${availability.available ? "text-green-600" : "text-[#E5312A]"}`}
                >
