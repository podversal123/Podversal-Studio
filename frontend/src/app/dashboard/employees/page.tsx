  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => setShowModal(true)} className="btn-primary flex items-center gap-2 w-auto">
      ...
      {loading ? (
        ...
        <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] text-center py-16">
          ...
          <p className="text-sm text-[#6b6b6b] dark:text-[#8a8a8a]">Click "Add Employee" to get started.</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-[#111] border border-[#e5e5e5] dark:border-[#2a2a2a] w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e5e5e5] dark:border-[#2a2a2a]">
              <h2 className="text-base font-black text-gray-900 dark:text-white">Add Employee</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black tracking-[0.15em] uppercase text-[#6b6b6b] dark:text-[#888] mb-2">Job Title *</label>
                <input {...register('jobTitle')} className="input-field" placeholder="e.g. Studio Technician" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {errors.shiftStart && <p className="text-[#E5312A] text-xs mt-1">{errors.shiftStart.message}</p>}
                </div>
                <div>
                  <label className="block text-[10px] font-black tracking-[0.15em] uppercase text-[#6b6b6b] dark:text-[#888] mb-2">Shift End *</label>
                  <input {...register('shiftEnd')} type="time" className="input-field" />
                  {errors.shiftEnd && <p className="text-[#E5312A] text-xs mt-1">{errors.shiftEnd.message}</p>}
                </div>
              </div>
