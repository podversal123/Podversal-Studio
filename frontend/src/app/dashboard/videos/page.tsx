...
import { Plus, Edit, Trash2, Eye, EyeOff, Play, Upload, Youtube } from 'lucide-react';
...
const EMPTY = {
  ...
  cloudinaryUrl: '', thumbnailUrl: '', category: 'General',
...
export default function VideosAdminPage() {
  ...
  const openVideoUpload = () => {
    ...
    cldUpload(
      { folder: 'studio-videos', resourceType: 'video', multiple: false, sources: ['local', 'url'] },
      (info) => {
      ...
      () => { setUploadingVideo(false); toast.error('Upload failed'); },
    );
  };

  const handleSave = async () => {
    if (!form.title) { toast.error('Title is required'); return; }
    if (videoSource === 'youtube' && !form.youtubeId) { toast.error('YouTube video ID is required'); return; }
    if (videoSource === 'cloudinary' && !form.cloudinaryUrl) { toast.error('Upload a video first'); return; }
    setSaving(true);
    try {
      const payload = {
        title:         form.title,
        description:   form.description  || undefined,
        youtubeId:     videoSource === 'youtube'    ? form.youtubeId     || undefined : undefined,
        cloudinaryUrl: videoSource === 'cloudinary' ? form.cloudinaryUrl || undefined : undefined,
  ...
  const handleToggle = async (v: StudioVideo) => {
    setVideos(prev => prev.map(x => x.id === v.id ? { ...x, isPublished: !x.isPublished } : x));
    try {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-black text-gray-900 dark:text-white">Studio Videos</h1>
          <p className="text-sm text-[#6b6b6b] dark:text-[#8a8a8a] mt-1">
            {videos.filter(v => v.isPublished).length} visible · {videos.filter(v => !v.isPublished).length} hidden
          </p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1,2,3].map(i => <div key={i} className="h-48 bg-[#f5f5f5] dark:bg-[#1a1a1a] animate-pulse" />)}
        </div>
      ) : videos.length === 0 ? (
        <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] text-center py-16">
          <Play size={40} className="mx-auto mb-3 text-[#e5e5e5] dark:text-[#3a3a3a]" />
          <p className="font-bold text-gray-900 dark:text-white mb-1">No videos yet</p>
          <p className="text-sm text-[#6b6b6b] dark:text-[#8a8a8a]">Add YouTube IDs or upload videos from Cloudinary</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {videos.map(v => (
            <div key={v.id} className="border border-[#e5e5e5] dark:border-[#2a2a2a] bg-white dark:bg-[#111111] overflow-hidden">
              <div className="relative aspect-video bg-[#f5f5f5] dark:bg-[#1a1a1a]">
                {(() => {
                  if (v.cloudinaryUrl) {
                    return <CldVideoThumb src={v.cloudinaryUrl} className="w-full h-full object-cover" />;
                  }
                  ...
                  return thumb ? (
                    <img src={thumb} alt={v.title} className="w-full h-full object-cover"
                      onError={e => {
                        if (v.youtubeId && e.currentTarget.src.includes('maxresdefault')) {
                ...
                <span className={`absolute top-2 left-2 text-[10px] font-black tracking-[0.1em] uppercase px-2 py-0.5 ${v.isPublished ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400'}`}>
              ...
              <div className="p-3">
                <h3 className="font-bold text-gray-900 dark:text-white text-sm line-clamp-1">{v.title}</h3>
                <p className="text-[10px] text-[#aaa] dark:text-[#555] mt-0.5">{v.category}</p>
                <div className="flex items-center gap-1.5 mt-3">
                  <button onClick={() => handleToggle(v)} title={v.isPublished ? 'Hide' : 'Show'} className="p-1.5 hover:bg-[#f5f5f5] dark:hover:bg-[#1a1a1a] text-[#6b6b6b] dark:text-[#8a8a8a] transition-colors">
                    {v.isPublished ? <EyeOff size={14} /> : <Eye size={14} />}
                  </button>
                  {v.youtubeId && (
                    <a href={`https://www.youtube.com/watch?v=${v.youtubeId}`} target="_blank" rel="noreferrer" className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 text-[#E5312A] transition-colors" title="Watch on YouTube">
                      <Play size={14} />
                    </a>
                  )}
                  <button onClick={() => openEdit(v)} className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 transition-colors" title="Edit">
                    <Edit size={14} />
                  </button>
                  <button onClick={() => handleDelete(v.id, v.title)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 text-[#E5312A] transition-colors" title="Remove">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-white dark:bg-[#111111] w-full max-w-lg border border-[#e5e5e5] dark:border-[#2a2a2a] max-h-[90vh] flex flex-col">
            ...
            <div className="px-5 py-4 border-b border-[#e5e5e5] dark:border-[#2a2a2a] flex items-center justify-between flex-shrink-0">
              ...
              <button onClick={() => setModal(null)} className="text-[#aaa] hover:text-gray-900 dark:hover:text-white text-lg leading-none">✕</button>
            </div>
            ...
            <div className="p-5 space-y-4 overflow-y-auto flex-1">
              ...
              {/* Video source toggle */}
              <div>
                <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Video Source</label>
              ...
              {videoSource === 'youtube' && (
                <div>
                  <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">YouTube Video ID *</label>
                  ...
                  <p className="text-[10px] text-[#aaa] dark:text-[#555] mt-1">youtube.com/watch?v=<strong>dQw4w9WgXcQ</strong> — copy only the ID part</p>
                </div>
              )}
              ...
              {videoSource === 'cloudinary' && (
                <div>
                  <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Video File *</label>
                  {form.cloudinaryUrl ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between border border-[#e5e5e5] dark:border-[#2a2a2a] px-3 py-2">
                        <p className="text-xs font-bold text-green-600 dark:text-green-400">Video uploaded</p>
                        <button type="button" onClick={openVideoUpload} className="text-xs text-[#E5312A] hover:underline font-bold">Replace</button>
                      </div>
                    </div>
                  ) : (
                    <button
                    ...
                    >
                      ...
                      <span className="text-sm font-bold text-gray-900 dark:text-white">{uploadingVideo ? 'Opening widget…' : 'Upload Video'}</span>
                      <span className="text-xs text-[#aaa]">MP4, MOV, AVI supported</span>
                    </button>
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Sort Order</label>
              ...
              {/* Thumbnail */}
              <div>
                <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Custom Thumbnail URL <span className="font-normal normal-case">(optional)</span></label>
                <input value={form.thumbnailUrl} onChange={e => setForm(f => ({ ...f, thumbnailUrl: e.target.value }))} className="input-field" placeholder="Leave empty — YouTube auto-thumbnail used for YouTube videos" />
              </div>

              {/* Description */}
              <div>
                <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Description <span className="font-normal normal-case">(optional)</span></label>
                <textarea rows={2} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className="input-field resize-none" placeholder="Short description" />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-[#6b6b6b] dark:text-[#8a8a8a]">{form.isPublished ? 'Published (visible on site)' : 'Draft (hidden)'}</span>
              </div>
            </div>
