  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <p className="text-[10px] font-black tracking-[0.2em] uppercase text-[#E5312A] mb-1">Studio Schedule</p>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white">Calendar</h1>
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-2">
          {Object.entries(SERVICE_LABELS).map(([type, label]) => (
            <div key={type} className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 flex-shrink-0" style={{ backgroundColor: SERVICE_COLORS[type] }} />
              <span className="text-[11px] font-bold text-[#6b6b6b] dark:text-[#8a8a8a]">{label}</span>
            </div>
          ))}
        </div>
      </div>
