  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <p className="text-[10px] font-black tracking-[0.2em] uppercase text-[#E5312A] mb-1">Customer Management</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#f0f0f0] dark:divide-[#1e1e1e]">
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="text-center text-[#aaa] dark:text-[#555] py-10">
                    ...
                    <tr
                      ...
                      className={`cursor-pointer hover:bg-[#fafafa] dark:hover:bg-[#161616] transition-colors ${
                        selected?.id === c.id ? 'bg-[#E5312A]/5 dark:bg-[#E5312A]/10' : ''
                      }`}
                    >
                      <td className="py-3 px-4">
                        <p className="font-bold text-gray-900 dark:text-white">{c.user?.name ?? '—'}</p>
                        <p className="text-[10px] text-[#aaa] dark:text-[#555] sm:hidden mt-0.5">{c.user?.email}</p>
                      </td>
                      <td className="py-3 px-4 hidden sm:table-cell">
                        <p className="text-[#6b6b6b] dark:text-[#8a8a8a]">{c.user?.email ?? '—'}</p>
                        {c.user?.phone && <p className="text-[10px] text-[#aaa] dark:text-[#555] mt-0.5">{c.user.phone}</p>}
                      </td>
            ...
            {selected ? (
              <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] bg-white dark:bg-[#111111] space-y-4 p-5">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="font-black text-gray-900 dark:text-white">{selected.user?.name}</h2>
                    <p className="text-xs text-[#6b6b6b] dark:text-[#8a8a8a] mt-0.5">
                      {selected.companyName ?? selected.category ?? 'Individual'}
                    </p>
                  </div>
                  <button
                ...
                <div className="space-y-2 text-sm border-t border-[#f0f0f0] dark:border-[#1e1e1e] pt-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white text-right truncate ml-3">{selected.user?.email}</span>
                  </div>
                  {selected.user?.phone && (
                    <div className="flex justify-between">
                      <span className="text-[#aaa] dark:text-[#555]">Phone</span>
                      <span className="font-bold text-gray-900 dark:text-white">{selected.user.phone}</span>
                    </div>
                  )}
                  {selected.gstNumber && (
                    <div className="flex justify-between">
                      <span className="text-[#aaa] dark:text-[#555]">GST</span>
                      <span className="font-bold text-gray-900 dark:text-white font-mono text-xs">{selected.gstNumber}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-[#aaa] dark:text-[#555]">Category</span>
                    <span className="font-bold text-gray-900 dark:text-white">{selected.category ?? 'INDIVIDUAL'}</span>
                  </div>
                </div>
                  ...
                  {stats ? (
                    <div className="grid grid-cols-3 gap-2">
                      <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] p-2 text-center">
                        <div className="text-xl font-black text-gray-900 dark:text-white">{stats.totalBookings}</div>
                        <div className="text-[10px] text-[#aaa] dark:text-[#555] uppercase tracking-wide mt-0.5">Total</div>
                      </div>
                      <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] p-2 text-center">
                        <div className="text-xl font-black text-green-600 dark:text-green-400">{stats.completedBookings}</div>
                        <div className="text-[10px] text-[#aaa] dark:text-[#555] uppercase tracking-wide mt-0.5">Done</div>
                      </div>
                      <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] p-2 text-center">
                        <div className="text-sm font-black text-[#E5312A]">₹{Number(stats.totalSpent).toLocaleString('en-IN')}</div>
                        <div className="text-[10px] text-[#aaa] dark:text-[#555] uppercase tracking-wide mt-0.5">Spent</div>
                      </div>
                    </div>
                  ) : (
                    <div className="grid grid-cols-3 gap-2">
                      {[1,2,3].map(i => <div key={i} className="h-12 bg-[#f5f5f5] dark:bg-[#1a1a1a] animate-pulse" />)}
                    </div>
                  )}
                </div>
                ...
                {selected.bookings?.length > 0 && (
                  <div className="border-t border-[#f0f0f0] dark:border-[#1e1e1e] pt-3 space-y-1">
                    <p className="text-[10px] font-black tracking-[0.1em] uppercase text-[#aaa] dark:text-[#555] mb-2">Recent Bookings</p>
                    {selected.bookings.slice(0, 5).map((b: any) => (
                      <div key={b.id} className="flex justify-between text-xs">
                        <span className="font-bold text-gray-900 dark:text-white">{b.bookingCode}</span>
                ...
                <div className="border-t border-[#f0f0f0] dark:border-[#1e1e1e] pt-3 space-y-2">
                  <p className="text-[10px] font-black tracking-[0.1em] uppercase text-[#aaa] dark:text-[#555]">Internal Notes</p>
                  <textarea
                    rows={3}
                    value={notes}
              ...
              <div className="border border-[#e5e5e5] dark:border-[#2a2a2a] text-center py-16">
                <p className="text-sm text-[#aaa] dark:text-[#555]">Select a customer to view details</p>
              </div>
            )}
          </div>
        </div>
