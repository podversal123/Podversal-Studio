    ...
    <div className="space-y-6">
      ...
      <div className="border border-[#e5e5e5] dark:border-[#2a2a2a]">
        ...
        <div className="px-5 py-4 border-b border-[#e5e5e5] dark:border-[#2a2a2a] bg-[#f9f9f9] dark:bg-[#161616]">
          <p className="text-[10px] font-black tracking-[0.15em] uppercase text-[#aaa] dark:text-[#555]">Service Pricing</p>
        </div>
                ...
                {services.map(svc => (
                  <tr key={svc.id} className="hover:bg-[#fafafa] dark:hover:bg-[#161616] transition-colors">
                    <td className="px-5 py-4">
                      ...
                      <p className="text-[10px] font-black tracking-[0.08em] uppercase text-[#aaa] dark:text-[#555] mt-0.5">
                        {SERVICE_TYPE_LABELS[svc.type] ?? svc.type}
                      </p>
                    </td>
                    <td className="px-5 py-4 hidden sm:table-cell max-w-xs">
                      <p className="line-clamp-2 text-xs text-[#6b6b6b] dark:text-[#8a8a8a]">{svc.description}</p>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right text-[#6b6b6b] dark:text-[#8a8a8a]">
                      {svc.minDuration}h
                    </td>
                    <td className="px-5 py-4 text-center">
                      <span className={`text-[10px] font-black tracking-[0.06em] uppercase px-2 py-1 ${
                        svc.isActive
                          ...
                          : 'bg-[#f5f5f5] dark:bg-[#2a2a2a] text-[#aaa] dark:text-[#555]'
                      }`}>
                    ...
                    <td className="px-5 py-4">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-bold text-[#6b6b6b] dark:text-[#8a8a8a] hover:text-[#E5312A] dark:hover:text-[#E5312A] transition-colors ml-auto"
                      >
                        <Pencil size={13} /> Edit
                      </button>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="w-full max-w-lg bg-white dark:bg-[#111111] border border-[#e5e5e5] dark:border-[#2a2a2a]">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e5e5e5] dark:border-[#2a2a2a] bg-[#f9f9f9] dark:bg-[#161616]">
              ...
              <button onClick={() => setEditing(null)} className="text-[#aaa] hover:text-gray-900 dark:hover:text-white transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit(onSave)} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Display Name</label>
                <input {...register('name')} className="input-field" />
                {errors.name && <p className="text-[#E5312A] text-xs mt-1">{errors.name.message}</p>}
              </div>

              <div>
                <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Description</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-black tracking-[0.12em] uppercase text-[#aaa] dark:text-[#555] mb-1.5">Min Duration (hrs)</label>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  ...
                  className="px-4 py-3 border border-[#e5e5e5] dark:border-[#2a2a2a] text-sm font-bold text-[#6b6b6b] dark:text-[#8a8a8a] hover:border-gray-400 transition-colors"
                >
                  Cancel
                </button>
              </div>
