...
export class BookingsService {
  ...
  async create(dto: CreateBookingDto, createdById: string, userRole: Role) {
    ...
    try {
      ...
      const totalAmount = Math.round(
        durationHours * Number(service.pricePerHour),
      );
  ...
  findAll(
    userId: string,
    userRole: Role,
    query: { date?: string; status?: BookingStatus },
  ) {
    const where: any = {};
  ...
  async sendQuote(id: string, dto: SendQuoteDto) {
    ...
    if (
      ...
      throw new BadRequestException(
        "Quote can only be sent for bookings in REQUEST or CHECKING status",
      );
    }
    ...
    this.notifications
      .sendBookingNotification(id, "QUOTE_SENT")
      .catch(() => {});
  ...
  async approve(id: string) {
    ...
    this.notifications
      .sendBookingNotification(id, "BOOKING_APPROVED")
      .catch(() => {});
    this.sse.emit({
  ...
  async cancel(id: string, userId: string, userRole: Role) {
    ...
    if (userRole === Role.CUSTOMER) {
      ...
      if (
        ...
        throw new BadRequestException(
          "Cannot cancel a booking after advance payment",
        );
      }
    }
    ...
    this.notifications
      .sendBookingNotification(id, "BOOKING_CANCELLED")
      .catch(() => {});
    this.sse.emit({
      type: "booking.updated",
      bookingId: id,
  ...
  async markInProgress(id: string) {
    ...
    if (booking.status !== BookingStatus.ADVANCE_PAID) {
      throw new BadRequestException(
        "Booking must have advance paid before starting",
      );
    }
    const updated = await this.prisma.booking.update({
      where: { id },
  ...
  async markCompleted(id: string) {
    ...
    if (booking.status !== BookingStatus.IN_PROGRESS) {
      throw new BadRequestException(
        "Booking must be IN_PROGRESS to mark as completed",
      );
    }
    const updated = await this.prisma.booking.update({
      where: { id },
  ...
  private validateSlot(startTime: string, endTime: string) {
    ...
    if ((end - start) % MIN_DURATION !== 0) {
      throw new BadRequestException(
        "Booking duration must be in 2-hour increments (2h, 4h, 6h, ...)",
      );
    }
  }
