...
export class InvoicesService {
  ...
  async generate(
    ...
    this.redis
      .set(
        `pdf:invoice:${PDF_CACHE_VERSION}:${invoice.id}`,
        pdfBuffer.toString("base64"),
        PDF_CACHE_TTL,
      )
      .catch(() => {
        /* non-critical  streamPdf will rebuild on cache miss */
      });
    ...
    this.sendInvoiceEmail(
      ...
      console.error(
        `[Invoice Email Failed] ${invoice.invoiceNumber}: ${err?.message ?? err}`,
      ),
    );
  ...
  async streamPdf(
    ...
    if (requesterRole === "CUSTOMER") {
      const owns =
        invoice.booking.createdById === requesterId ||
        invoice.booking.customer?.userId === requesterId;
      if (!owns) throw new ForbiddenException("Access denied");
    }
    ...
    this.redis
      .set(cacheKey, buffer.toString("base64"), PDF_CACHE_TTL)
      .catch(() => {
        /* non-critical */
      });
...
<body bgcolor="#f5f5f5">
  ...
  private buildInvoicePdf(data: {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (data.logoBuffer) {
        ...
        doc.text(`Date: ${new Date().toLocaleDateString("en-IN")}`, LM, 74, {
          width: W,
          align: "right",
        });
        doc
        ...
        doc.text(`Date: ${new Date().toLocaleDateString("en-IN")}`, LM, 85, {
          width: W,
          align: "right",
        });
        doc
      ...
      doc
        ...
        .text(booking.service?.name ?? "Studio Session", C2, y, {
          width: W / 2 - 10,
        });
      ...
      doc
        ...
        .text(
          `Date: ${new Date(booking.shootDate).toLocaleDateString("en-IN")}`,
          C2,
          y,
          { width: W / 2 - 10 },
        );
      ...
      y += 14;
      doc
        .text(booking.customerPhone ?? "", LM, y, { width: W / 2 - 10 })
        .text(`Time: ${booking.startTime} – ${booking.endTime}`, C2, y, {
          width: W / 2 - 10,
        });
      ...
      doc.text(`Duration: ${booking.durationHours} hrs`, C2, y, {
        width: W / 2 - 10,
      });
      ...
      // ── Table row ────────────────────────────────────────
      y += 26;
      const ratePerHr = booking.durationHours
        ? amount / booking.durationHours
        : 0;
      doc
        .fontSize(10)
        .font("Helvetica")
        .fillColor("#1a1a1a")
        .text(`${booking.service?.name ?? "Studio"} Session`, LM + 8, y + 8, {
          width: 250,
        })
        .text(`${booking.durationHours} hrs`, LM + 260, y + 8, {
          width: 70,
          align: "right",
        })
      ...
      if (booking.discountAmount && booking.discountAmount > 0) {
        doc
          ...
          .fillColor("#dc2626")
          .text(`-${fmt(booking.discountAmount)}`, LM + 420, y + 8, {
            width: 68,
            align: "right",
          });
        y += 26;
  ...
  private async sendInvoiceEmail(
    ...
    if (res.ok) {
      ...
      console.error(
        `[Invoice Email Failed] ${invoiceNumber}: ${await res.text()}`,
      );
    }
  }
}
