...
export class AgentsController {
  ...
  @UseGuards(RolesGuard)
  @Roles('SUPER_ADMIN', 'STUDIO_MANAGER', 'REFERRAL_AGENT')
  commissionSummary(@Param('id') id: string) {
  ...
  @UseGuards(RolesGuard)
  @Roles('SUPER_ADMIN', 'STUDIO_MANAGER', 'REFERRAL_AGENT')
  async commissionStatement(@Param('id') id: string, @Res() res: Response) {
    const pdf = await this.agents.generateCommissionStatement(id);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="commission-statement-${id}.pdf"`);
    res.send(pdf);
  }
}
